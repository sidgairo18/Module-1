...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        if not isinstance(local_deris, tuple) and not isinstance(local_deris, list):
            local_deris = (local_deris,)
...
def backpropagate(final_variable_with_deriv):
    ...
    is_in_queue = {}
    is_in_queue[final_variable_with_deriv.variable.name] = final_variable_with_deriv.variable
    while len(queue) > 0:
        curr_var = queue.popleft()
        v = curr_var.variable
        deriv = curr_var.deriv
        del is_in_queue[v.name]
        if is_leaf(v):
            v._add_deriv(deriv)
            continue

        for vwd in v.history.backprop_step(deriv):
            if vwd.variable.name in is_in_queue:
                for idx, _ in enumerate(queue):
                    if queue[idx].variable.name == vwd.variable.name:
                        queue[idx].deriv += vwd.deriv
                        break

            else:
                is_in_queue[vwd.variable.name] = vwd.variable
                queue.append(vwd)
