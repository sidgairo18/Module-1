...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    vals = list(vals)
    vals[arg] += epsilon
    fxph = f(*vals) # value of f(x+h)
    vals[arg] -= 2*epsilon
    fxmh = f(*vals) # value of f(x-h)

    return (fxph-fxmh)/(2*epsilon) # return {f(x+h) - f(x-h)} / 2*h
...
class Sigmoid(ScalarFunction):
    ...
    def backward(ctx, d_output):
        ...
        x = operators.mul(x, 1.0-x)
        return operators.mul(x, d_output) 
